...
//	MPU6050_Init();
	...
	gpio_init(GPIO_B, Pin_1, OUT_PP);             // PB1 蜂鸣器 (低电平触发)
	gpio_set(GPIO_B, Pin_1, 1);                   // 默认高电平=关闭
	...
	while (1)
	{
		...
		if (g_turn_active) {
			// ====== 直角弯调试显示 ======
			// Line2: 方向 + 帧数  示例: "TURN R 05"
			OLED_ShowString(2, 1,  "TURN ");
			OLED_ShowString(2, 6,  (g_turn_dir_char == 'L') ? "L" : "R");
			OLED_ShowString(2, 7,  " ");
			OLED_ShowNum   (2, 8,  g_turn_frame_cnt, 2);

			// Line3: 传感器模式 + 轮速  示例: "L3/R4 S20/00"
			OLED_ShowString(3, 1,  "L");
			OLED_ShowNum   (3, 2,  g_turn_lcnt, 1);
			OLED_ShowString(3, 3,  "/R");
			OLED_ShowNum   (3, 5,  g_turn_rcnt, 1);
			OLED_ShowString(3, 7,  "S");
			OLED_ShowNum   (3, 8,  g_turn_speed_L, 2);
			OLED_ShowString(3, 10, "/");
			OLED_ShowNum   (3, 11, g_turn_speed_R, 2);

			// Line4: 编码器实际速度 (验证内轮是否真停了)
			OLED_ShowString   (4, 1,  "E");
			OLED_ShowSignedNum(4, 2,  (int)motorA.now, 4);
			OLED_ShowString   (4, 7,  "/");
			OLED_ShowSignedNum(4, 8,  (int)motorB.now, 4);
		} else {
		// Line2: 巡线状态 + 目标速度
		...
		delay_ms(10);  // v2.1: 10ms → 100Hz采样, 消除高速直角弯漏检盲区
	}
}
