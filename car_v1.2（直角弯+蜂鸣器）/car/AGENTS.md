# AGENTS.md — car_v2.0 智能小车巡线控制系统（直角弯）

## 项目概述

基于 STM32F103C8T6 的智能小车巡线系统 v2.0，核心挑战是**直角弯**（90° 急转弯）。具备 **编码器速度闭环 PID**、MPU6050 姿态感知、HMC5883L 磁力计、SSD1306 OLED 显示。

**关键硬件**: TB6612 电机驱动 ×2、编码器 ×2、8 路灰度传感器（亚博智能）、MPU6050、HMC5883L。

**v2.0 核心目标**: 在不重新编译的前提下，通过 UART3 实时调参找到能稳定过直角弯的参数组合。
...
### 统一头文件 `headfile.h`
**每个 `.c` 文件只 `#include "headfile.h"` 一行**，不再单独 include 其他头文件。
新增模块 → 在 `headfile.h` 加 `#include "xxx.h"`。
...
```
命名: 全局变量用 `g_` 前缀（`g_sensor_data`, `g_track_kp`），模块实例用 PascalCase（`motorA`, `KF_Yaw`）。

### 编码：GB2312
本项目源码为 **GB2312** 编码（非 UTF-8）。UTF-8 中文会在 Keil 编译时报 `invalid multibyte character sequence`。
含 >0x7F 字节的文件（如 OLED 字库）禁止用 VS Code 编辑，会破坏编码。

### 命名规范速查
| 层级 | 风格 | 示例 |
|------|------|------|
| `ml_libs/` 库函数 | `snake_case` | `gpio_init()`, `pwm_init()`, `uart_sendbyte()` |
| `code/` 传感器驱动 | `PascalCase_with_underscores` | `Grayscale_Sensor_Init()`, `Grayscale_Sensor_Read_All()` |
| `code/` 应用层 | `snake_case` | `motor_target_set()`, `pid_cal()`, `pid_control()` |
| 全局变量 | `g_` 前缀 | `g_sensor_data`, `g_track_kp` |
| 结构体类型 | `snake_t` 后缀 | `pid_t`, `KF_t` |
| 宏/常量 | `UPPER_CASE` | `MAX_DUTY`, `TURN_TIMEOUT` |
| 头文件保护 (`code/`) | `__filename_h_` | `__gray_track_h_` |
| 头文件保护 (`ml_libs/`) | `_filename_h_` | `_ml_gpio_h_` |
...
## 巡线算法（gray_track.c）— 两状态：NORMAL + TURNING

> 详细逐帧分析见 [循线控制逻辑详解.md](循线控制逻辑详解.md)

### 正常巡线（NORMAL 状态）
直道段使用**加权差比和 + PD**：
1. 8 路传感器加权求和：`error = sum_L - sum_R`，范围 ±12，不除 cnt
2. 转向 PD：`steer_out = Kp*error + Kd*(error - prev_error)`，限幅 ±60
3. 差速分配：`L = base - steer_out, R = base + steer_out`，弯道随 error 减速
4. 全白丢线 → FIND 状态（保持上一帧 error 走惯性）
5. 全黑 → CROSS 状态（十字路口）

运行时 OLED 状态: `GO / R1~R3 / L1~L3 / FIND / CROSS / TURN`

### 直角弯（TURNING 状态）— 传感器物理特征触发

直角弯不依赖 error 阈值，而是检测直角弯路口的**物理传感器特征**：
- **触发**：一侧 ≥3 灯同时踩线，另一侧 ≤1（容忍竖线残留）
- **执行**：内轮停转，外轮低速（`g_speed_slow=20`），前 3 帧禁止退出
- **退出**：最外侧单灯独亮（D1 only 或 D8 only），超时 40 帧强制退出
...
```
                 ...
                 │  (加权差比和 + PD 巡线)       │
                 └──────────┬──────────────────┘
                            │ right_cnt>=3 && left_cnt<=1
                            │ 或 left_cnt>=3 && right_cnt<=1
                            ▼
                 ┌─────────────────────────────┐
                 │         TURNING              │
                 │  内轮=0, 外轮=g_speed_slow   │
                 │  前3帧禁止退出               │
                 │  退出: D1 only / D8 only     │
                 │  超时: 40帧 (~1.6s)          │
                 └──────────┬──────────────────┘
...
| 参数 | 位置 | 默认值 | 作用 |
|------|------|--------|------|
| `left_turn_trigger` | `track()` L93 | `left_cnt>=3 && right_cnt<=1` | 左转触发 |
| `right_turn_trigger` | `track()` L94 | `right_cnt>=3 && left_cnt<=1` | 右转触发 |
| 最小转弯帧 | `track()` L113 | `turn_frame <= 3` | 禁止过早退出 |
| `TRACK_SPEED_SLOW` | `gray_track.c` L25 | 20 | 转弯时外轮速度 |
| `TURN_TIMEOUT` | `gray_track.c` L40 | 40 帧 | 超时强制退出 |

## 已知问题与陷阱

| 问题 | 位置 | 说明 |
|------|------|------|
| `gpio_init` 枚举冲突 | `grayscale_sensor.c:37` | IF 枚举值与 OUT_PP 冲突，PA15 浮空输入直接用寄存器操作 |
| `speed_now` 未使用 | `motor.c:11` | 声明但无代码读写 |
| HMC5883L 未初始化 | `isr.c:79` | 磁力计驱动已注释，`yaw_hmc` 可能读无效值 |
| MPU6050 初始化被注释 | `main.c:22` | `MPU6050_Init()` 被注释，陀螺仪数据未启用 |
| 角度 PID 环废弃 | `pid.c:56-63` | `angle` 实例声明但初始化/计算全部注释 |
| printf 与 datavision_send 互斥 | UART1 | 二者共用 UART1，不能同时开启 |
| GB2312 编码 | 全局 | UTF-8 中文编译报错，含 >0x7F 字节文件禁 VS Code 编辑 |

## 修改代码后必检清单

参考仓库记忆 checklist：
- 新增变量 → 在对应 `.h` 加 `extern`
- 新增 `.c` → 加入 Keil 工程 + 在 `headfile.h` 加 `#include`
- 改完立即让用户编译验证，不攒批
- 残留字符检查（防 "zu"、"xxx" 等手滑产物）
...
```c
#define TRACK_SPEED_FAST    30   // 直道/基准速度 (直角弯拐不过先降这个)
#define TRACK_SPEED_SLOW    18   // 丢线搜索速度 (直角弯丢线期保持低速)
#define TRACK_SPEED_MIN      2   // 弯道base保底速度 (直角弯可降到0~1)
float g_track_kp = 5.8f;        // 转向P: 直角弯建议 6~10
float g_track_kd = 3.0f;        // 转向D: 直角弯建议 1~3（低D快速响应）
#define STEER_MAX        60     // 转向输出上限 (直角弯可试 70~80)
```

**直角弯快速起步参数**（UART3 一键设置）:
```
S30 KP8 KD2 P300
```
含义：直道速度 30 / 转向 P=8 / 转向 D=2（低阻尼快响应）/ 电机 P=300（强动力）
